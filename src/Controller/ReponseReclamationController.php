...
class ReponseReclamationController extends AbstractController
{
   ...
   #[Route('/{idRec}', name: 'app_reponse_reclamation_new', methods: ['GET', 'POST'])]
    public function new(Request $request, EntityManagerInterface $entityManager,$idRec,ReclamationRepository $reclamationRepository ): Response
    ...
    public function show(ReponseRepository $reponseRepository,$idReponse): Response
    {
        return $this->render('reponse_reclamation/show.html.twig', [
            'reponse_reclamation' => $reponseRepository->find($idReponse),
        ]);
    }

    #[Route('/edit/{idReponse}', name: 'app_reponse_reclamation_edit', methods: ['GET', 'POST'])]
    public function edit($idReponse,Request $request, ReponseRepository $reponseRepository, EntityManagerInterface $entityManager): Response
    {
        $reponseReclamation=$reponseRepository->find($idReponse);
        $form = $this->createForm(ReponseReclamationType::class, $reponseReclamation);
        $form->handleRequest($request);
    ...
    public function searchAction1(Request  $request,ReponseRepository $reponseRepository)
    {
        //  dd($request->query->all());
        $requestString = $request->query->get('k');

        $reponse_reclamations =  $reponseRepository->findEntitiesByStringReponse("$requestString");
    ...
    #[Route('/remove1/{idReponse}', name: 'app_reclamation_delete3', methods: ['GET','POST'])]
    public function delete2($idReponse, ReponseRepository $reponseRepository , EntityManagerInterface $entityManager): Response
    {

        $rec = $reponseRepository->find($idReponse);
